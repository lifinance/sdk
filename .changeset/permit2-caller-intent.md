...
- A step that carries typed data and receives no `transactionRequest` executes through the relayer. `batched` needs a transaction request and `standard` throws without one, so that is the only path a signature-only step can take. The rule is applied at dispatch time rather than in the strategy, because before the step is prepared a caller intent that will receive a transaction and one that never will look identical.
- The relayer never asks for a signature it already holds. An entry already in `signedTypedData` — a caller intent signed before prepare, for example — is relayed with that signature instead of being signed a second time, so a signature-only step raises one wallet prompt and not two.
