...
Three things that sentence has to get right, all discovered during implementation:

- The old text claimed 422 mapped to `TransactionConflict` "across all endpoints". That is what Task 7 removed, so leaving it would document the defect as a feature.
- The cause **type** changed for a funding 422, not just the code: a consumer previously caught an `HTTPError` carrying `status` and code 1020; it now catches a `ValidationError` with no `status` and code 1001. No in-repo caller depends on that, but it is consumer-visible and belongs in the note.
- `sleep(ms, signal?)` is a widened public signature (`sleep` is exported from `packages/sdk/src/index.ts`). Additive, so **minor** still covers it — but it must be stated.
