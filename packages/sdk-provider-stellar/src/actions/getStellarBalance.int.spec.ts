...
describe.skip.sequential('Stellar token balance', async () => {
  ...
  it(
    'should work for native XLM and USDC',
  ...
  it(
    'should return the token without an amount for an unreadable contract',
    { retry: retryTimes, timeout },
    async () => {
      ...
      const tokenBalances = await getStellarBalance(client, defaultWalletAddress, [
        findDefaultToken(CoinKey.XLM, ChainId.XLM),
        invalidToken,
      ] as Token[])
