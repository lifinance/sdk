...
describe('getTronBalance', () => {
  ...
  it('returns an empty array when no tokens are provided', async () => {
    const result = await getTronBalance(makeClient(), 'TJRabPrwbZy45sbavfcjinPJC18kjpRTv8', [])
  ...
  it('warns when tokens span multiple chainIds', async () => {
    ...
    const tokens = [
      { chainId: 728126428, address: '0x0', decimals: 6, symbol: 'a', name: 'a', priceUSD: '0', coinKey: 'a' },
      { chainId: 1, address: '0x0', decimals: 6, symbol: 'b', name: 'b', priceUSD: '0', coinKey: 'b' },
    ] as unknown as Token[]
    ...
    await getTronBalance(makeClient(), 'TJRabPrwbZy45sbavfcjinPJC18kjpRTv8', tokens).catch(() => {})

    expect(warn).toHaveBeenCalledWith('Requested tokens have to be on the same chain.')
  })
})
