...
describe('getBalances integration tests', () => {
  const loadAndCompareTokenAmounts = async (
    ...
    const tokenBalances = await getBalance(walletAddress, tokens as Token[])
  ...
  it(
    ...
    async () => {
      ...
      if (tokens?.length) {
        await loadAndCompareTokenAmounts(walletAddress, tokens.slice(0, 150)) // chunk limit is 100
      }
    },
    { retry: retryTimes, timeout }
  )
